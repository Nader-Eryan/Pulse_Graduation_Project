...
// }
import 'dart:async';
import 'package:pulse/Core/utils/notification_model.dart';
import 'package:sqflite/sqflite.dart';
import 'package:path/path.dart';
...
class USqlDb {
  ...
  // Insert notification using the model
  ...
  // Retrieve all notifications as a list of NotificationModel
  ...
  Future<int> updateIsTaken(int medId, String isTaken) async {
    Database? mydb = await db;
    int response = await mydb!.update(
        'meds',
        {
  ...
  Future<String?> getIsTaken(int medId) async {
    Database? mydb = await db;
    List<Map<String, dynamic>> response = await mydb!.query(
      'meds',
      columns: ['isTaken'],
    ...
    if (response.isNotEmpty) {
      return response.first['isTaken'] as String?;
    }
    return null;
  }
